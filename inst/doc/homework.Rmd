...
$$\hat{\theta}=\frac{\hat{\lambda}_1}{\sum_{j=1}^5\hat{\lambda}_j}$$ of $\theta$. Use bootstrap to estimate the bias and standard error of $\hat{\theta}$.
...
Conduct a Monte Carlo study to estimate the coverage probabilities of the standard normal bootstrap confidence interval, the basic bootstrap confidence interval, and the percentile confidence interval. Sample from a normal population and check the empirical coverage rates for the
 sample mean. Find the proportion of times that the confidence intervals miss on the left, and the porportion of times that the confidence intervals miss on the right.
...
下面考虑EM算法，有E步 (Expectation Step):
计算完全数据充分统计量的条件期望。对于指数分布，核心是计算 $X$ 在区间 $(u, v)$ 上的条件期望：
$$
...
当 EM 算法收敛时，参数不再变化，即 $\lambda^{(t+1)} = \lambda^{(t)} = \hat{\lambda}$。
将 $\hat{\lambda}$ 代入 M 步的迭代公式中：
...
此方程与MLE方法的 Score Equation 完全一致。
因此，EM 算法的收敛点 $\hat{\lambda}$ 满足似然方程，即 EM 算法收敛于观测数据的 MLE。
...
Rao [232, Sec. 5g] presented an example on genetic linkage of 197 animals in four categories (also discussed in [70, 110, 179, 280]). The group sizes are (125,18,20,34). Assume that the probabilities of the corresponding multinomial distribution are $$(\frac{1}{2}+\frac{\theta}{4},\,\frac{1-\theta}{4},\,\frac{1-\theta}{4},\, \frac{\theta}{4}).$$
...
使用 qqplot 比较 R 与 Rcpp 生成的样本，可以发现生成的样本点基本落在 y=x 附近，说明两种编码方式实现的方法得到的后验分布是相同的。而
使用 microbenchmark 比较运行时间，可以发现 Rcpp 版本远远快于 R 版本，说明使用C++能大幅提升算法计算速度。